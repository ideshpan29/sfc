...
tibcoforms.formCode['_G2B-gBTNEei_hctzr9GiyA'] = new Object();
tibcoforms.formCode['_G2B-gBTNEei_hctzr9GiyA']['defineActions'] = function() {
var fc = tibcoforms.formCode['_G2B-gBTNEei_hctzr9GiyA'];
    ...
    fc['rule_initrule1'] = function(formId, context, thisObj) {
	   try {
			...
			tibco.forms.Util.handleScriptAction.call(thisObj, formId, context, thisObj, "initrule1", "setVisibleSections", fc['action_setVisibleSections']);
	   } catch(e) {
...
pane.serviceDates02.setEnabled(true);
pane.contactDetails02.setEnabled(true);
pane.serviceRequest.setEnabled(true);
pane.pane8.setEnabled(true);
pane.Pane03.setEnabled(true);
pane.PneClientDetails.setEnabled(true);
pane.pne01.setEnabled(true);
	pane.TechnicalInfo.setEnabled(true);
	
}
else {
	pane.serviceDates02.setEnabled(false);
	pane.contactDetails02.setEnabled(false);
	pane.serviceRequest.setEnabled(false);
	pane.pane8.setEnabled(false);
	pane.Pane03.setEnabled(false);
	pane.PneClientDetails.setEnabled(false);
	pane.pne01.setEnabled(false);
	pane.TechnicalInfo.setEnabled(false);
}

			}
			fc['action_setVisibleSections'] = function(context, data, pane, control, factory, pkg, f , p) {
				 if (context.form && context.form._marked4Cancel)
					return;
				 var resource = context.form.resource;
			     var logger = tibcoforms.bridge.log_logger();
				pane.pne01.setVisible(true);
control.btnGeneralDetails.setLabel("General Details <");


pane.pane8.setVisible(true);
control.btnaddressDetails.setLabel("Address Details <");


pane.pane10.setVisible(true);
control.btncomments.setLabel("Comments >>");
			}
...
pane.TechnicalInfo_equipment01.getValue().add(Record);
...
if(data.getProvisioning_Data().getSiebelCircuitID()=='90981'){
    ...
    fc['rule_toggleGenericDetailsPane'] = function(formId, context, thisObj) {
	   try {
			tibco.forms.Util.handleScriptAction.call(thisObj, formId, context, thisObj, "toggleGenericDetailsPane", "toggleGenericDetailsPane", fc['action_toggleGenericDetailsPane']);
	   } catch(e) {
	       tibcoforms.bridge.log_error("Rule(toggleGenericDetailsPane) Error: " + e);
	       throw e;
	   }
	}

			fc['action_toggleGenericDetailsPane'] = function(context, data, pane, control, factory, pkg, f , p) {
				 if (context.form && context.form._marked4Cancel)
					return;
				 var resource = context.form.resource;
			     var logger = tibcoforms.bridge.log_logger();
				if(pane.pne01.getVisible()==true) {
	control.btnGeneralDetails.setLabel("Generic Details >>");
	pane.pne01.setVisible(false);
}
else {
	control.btnGeneralDetails.setLabel("Generic Details <<");
	pane.pne01.setVisible(true);
    ...
    fc['rule_toggleClientDetails'] = function(formId, context, thisObj) {
	   try {
			tibco.forms.Util.handleScriptAction.call(thisObj, formId, context, thisObj, "toggleClientDetails", "toggleClientDetails", fc['action_toggleClientDetails']);
	   } catch(e) {
	       tibcoforms.bridge.log_error("Rule(toggleClientDetails) Error: " + e);
	       throw e;
	   }
	}

			fc['action_toggleClientDetails'] = function(context, data, pane, control, factory, pkg, f , p) {
				 if (context.form && context.form._marked4Cancel)
					return;
				 var resource = context.form.resource;
			     var logger = tibcoforms.bridge.log_logger();
				if(pane.PneClientDetails.getVisible()==true) {
	control.btnclientDetails.setLabel("Client Details >>");
	pane.PneClientDetails.setVisible(false);
}
else {
	control.btnclientDetails.setLabel("Client Details <<");
	pane.PneClientDetails.setVisible(true);
}

			}
    fc['rule_toggleResource'] = function(formId, context, thisObj) {
	   try {
			tibco.forms.Util.handleScriptAction.call(thisObj, formId, context, thisObj, "toggleResource", "toggleResource", fc['action_toggleResource']);
	   } catch(e) {
	       tibcoforms.bridge.log_error("Rule(toggleResource) Error: " + e);
	       throw e;
	   }
	}

			fc['action_toggleResource'] = function(context, data, pane, control, factory, pkg, f , p) {
				 if (context.form && context.form._marked4Cancel)
					return;
				 var resource = context.form.resource;
			     var logger = tibcoforms.bridge.log_logger();
				if(pane.Pane03.getVisible()==true) {
	control.btnresource.setLabel("Resource >>");
	pane.Pane03.setVisible(false);
}
else {
	control.btnresource.setLabel("Resource <<");
	pane.Pane03.setVisible(true);
}

			}
    fc['rule_toggleAddressDetails'] = function(formId, context, thisObj) {
	   try {
			tibco.forms.Util.handleScriptAction.call(thisObj, formId, context, thisObj, "toggleAddressDetails", "toggleAddressDetails", fc['action_toggleAddressDetails']);
	   } catch(e) {
	       tibcoforms.bridge.log_error("Rule(toggleAddressDetails) Error: " + e);
	       throw e;
	   }
	}

			fc['action_toggleAddressDetails'] = function(context, data, pane, control, factory, pkg, f , p) {
				 if (context.form && context.form._marked4Cancel)
					return;
				 var resource = context.form.resource;
			     var logger = tibcoforms.bridge.log_logger();
				if(pane.pane8.getVisible()==true) {
	control.btnaddressDetails.setLabel("Address Details >>");
	pane.pane8.setVisible(false);
}
else {
	control.btnaddressDetails.setLabel("Address Details <<");
	pane.pane8.setVisible(true);
    ...
    fc['rule_toggleServiceRequest'] = function(formId, context, thisObj) {
	   try {
			tibco.forms.Util.handleScriptAction.call(thisObj, formId, context, thisObj, "toggleServiceRequest", "toggleServiceRequest", fc['action_toggleServiceRequest']);
	   } catch(e) {
	       tibcoforms.bridge.log_error("Rule(toggleServiceRequest) Error: " + e);
	       throw e;
	   }
	}

			fc['action_toggleServiceRequest'] = function(context, data, pane, control, factory, pkg, f , p) {
				 if (context.form && context.form._marked4Cancel)
					return;
				 var resource = context.form.resource;
			     var logger = tibcoforms.bridge.log_logger();
				if(pane.serviceRequest.getVisible()==true) {
	control.btnserviceRequest.setLabel("Service Request >>");
	pane.serviceRequest.setVisible(false);
}
else {
	control.btnserviceRequest.setLabel("Service Request <<");
	pane.serviceRequest.setVisible(true);
}

			}
    fc['rule_toggleContactDetails'] = function(formId, context, thisObj) {
	   try {
			tibco.forms.Util.handleScriptAction.call(thisObj, formId, context, thisObj, "toggleContactDetails", "toggleContactDetails", fc['action_toggleContactDetails']);
	   } catch(e) {
	       tibcoforms.bridge.log_error("Rule(toggleContactDetails) Error: " + e);
	       throw e;
	   }
	}

			fc['action_toggleContactDetails'] = function(context, data, pane, control, factory, pkg, f , p) {
				 if (context.form && context.form._marked4Cancel)
					return;
				 var resource = context.form.resource;
			     var logger = tibcoforms.bridge.log_logger();
				if(pane.contactDetails02.getVisible()==true) {
	control.btncontactDetails.setLabel("Contact Details >>");
	pane.contactDetails02.setVisible(false);
}
else {
	control.btncontactDetails.setLabel("Contact Details <<");
	pane.contactDetails02.setVisible(true);
}

			}
    fc['rule_toggleServiceDates'] = function(formId, context, thisObj) {
	   try {
			tibco.forms.Util.handleScriptAction.call(thisObj, formId, context, thisObj, "toggleServiceDates", "toggleServiceDates", fc['action_toggleServiceDates']);
	   } catch(e) {
	       tibcoforms.bridge.log_error("Rule(toggleServiceDates) Error: " + e);
	       throw e;
	   }
	}

			fc['action_toggleServiceDates'] = function(context, data, pane, control, factory, pkg, f , p) {
				 if (context.form && context.form._marked4Cancel)
					return;
				 var resource = context.form.resource;
			     var logger = tibcoforms.bridge.log_logger();
				if(pane.serviceDates02.getVisible()==true) {
	control.btnserviceDates.setLabel("Service Dates >>");
	pane.serviceDates02.setVisible(false);
}
else {
	control.btnserviceDates.setLabel("Service Dates <<");
	pane.serviceDates02.setVisible(true);
}

			}
    fc['rule_toggleComments'] = function(formId, context, thisObj) {
	   try {
			tibco.forms.Util.handleScriptAction.call(thisObj, formId, context, thisObj, "toggleComments", "toggleComments", fc['action_toggleComments']);
	   } catch(e) {
	       tibcoforms.bridge.log_error("Rule(toggleComments) Error: " + e);
	       throw e;
	   }
	}

			fc['action_toggleComments'] = function(context, data, pane, control, factory, pkg, f , p) {
				 if (context.form && context.form._marked4Cancel)
					return;
				 var resource = context.form.resource;
			     var logger = tibcoforms.bridge.log_logger();
				if(pane.pane10.getVisible()==true) {
	control.btncomments.setLabel("Comments >>");
	pane.pane10.setVisible(false);
}
else {
	control.btncomments.setLabel("Comments <<");
	pane.pane10.setVisible(true);
    ...
    fc['rule_toggleTechnicalInfo'] = function(formId, context, thisObj) {
	   try {
			tibco.forms.Util.handleScriptAction.call(thisObj, formId, context, thisObj, "toggleTechnicalInfo", "toggleTechnicalInfo", fc['action_toggleTechnicalInfo']);
	   } catch(e) {
	       tibcoforms.bridge.log_error("Rule(toggleTechnicalInfo) Error: " + e);
	       throw e;
	   }
	}

			fc['action_toggleTechnicalInfo'] = function(context, data, pane, control, factory, pkg, f , p) {
				 if (context.form && context.form._marked4Cancel)
					return;
				 var resource = context.form.resource;
			     var logger = tibcoforms.bridge.log_logger();
				if(pane.pneTechInfoParent.getVisible()==true) {
	control.btntechnicalInformation.setLabel("Technical Information >>");
    ...
    fc['rule_toggleCommentHistory'] = function(formId, context, thisObj) {
	   try {
			tibco.forms.Util.handleScriptAction.call(thisObj, formId, context, thisObj, "toggleCommentHistory", "toggleCommentHistory", fc['action_toggleCommentHistory']);
	   } catch(e) {
	       tibcoforms.bridge.log_error("Rule(toggleCommentHistory) Error: " + e);
	       throw e;
	   }
	}

			fc['action_toggleCommentHistory'] = function(context, data, pane, control, factory, pkg, f , p) {
				 if (context.form && context.form._marked4Cancel)
					return;
				 var resource = context.form.resource;
			     var logger = tibcoforms.bridge.log_logger();
				if(pane.CommentHistory.getVisible()==true) {
	control.btncommentHistory.setLabel("Comment History >>");
	pane.CommentHistory.setVisible(false);
}
else {
	control.btncommentHistory.setLabel("Comment History <<");
	pane.CommentHistory.setVisible(true);
}
...
tibcoforms.formCode['_G2B-gBTNEei_hctzr9GiyA']['defineActions']();

tibcoforms.formCode['_G2B-gBTNEei_hctzr9GiyA']['defineValidations'] = function() {
var fc = tibcoforms.formCode['_G2B-gBTNEei_hctzr9GiyA'];
...
fc['validation_TechnicalInfo_equipment01_TechnicalInfo_equipment01__multiplicity'] = function(formId, controlName, cloneUID, listIndex) {
	...
	// return tibco.forms.Util.handleInlineValidation.call(this, formId, this, cloneUID, listIndex, valScr, "TechnicalInfo_equipment01: TechnicalInfo_equipment01__multiplicity", true, false);
	return tibco.forms.Util.handleInlineValidation.call(this, formId, this, cloneUID, listIndex, valScr, "TechnicalInfo_equipment01__multiplicity", true, false);
}
...
tibcoforms.formCode['_G2B-gBTNEei_hctzr9GiyA']['defineValidations']();
